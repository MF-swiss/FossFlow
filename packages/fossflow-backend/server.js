...
const RATE_LIMIT_MAX = parsePositiveInt(process.env.RATE_LIMIT_MAX, 300);

const corsOptions =
  CORS_ORIGINS.length > 0
    ? {
        origin: (origin, callback) => {
          // Allow non-browser requests (no Origin header), and explicit allowlist origins.
...
const createApiRateLimiter = (windowMs, maxRequests) => {
...
const apiLimiter = createApiRateLimiter(RATE_LIMIT_WINDOW_MS, RATE_LIMIT_MAX);
...
app.listen(PORT, () => {
  ...
  console.log(
    `CORS: ${CORS_ORIGINS.length > 0 ? `RESTRICTED (${CORS_ORIGINS.join(', ')})` : 'OPEN'}`
  );
  console.log(
    `Rate limit: ${RATE_LIMIT_MAX} requests per ${Math.round(RATE_LIMIT_WINDOW_MS / 1000)}s window`
  );
  if (STORAGE_ENABLED) {
